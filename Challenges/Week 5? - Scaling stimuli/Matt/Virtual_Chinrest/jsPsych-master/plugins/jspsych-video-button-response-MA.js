...
jsPsych.plugins["video-button-response"] = (function() {
  ...
  plugin.info = {
    ...
    parameters: {
      ...
      response_only_after_video: {
        type: jsPsych.plugins.parameterType.BOOL,
        pretty_name: 'Only allow response after video ends',
        default: true,
        description: 'If true, the responses will be prevented until ther video ends.'
      },
      trial_duration: {
        type: jsPsych.plugins.parameterType.INT,
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.choices.length; i++) {
      ...
      video_html += '<div disabled class="jspsych-video-button-response-button" style="display: inline-block; margin:'+trial.margin_vertical+' '+trial.margin_horizontal+'" id="jspsych-video-button-response-button-' + i +'" data-choice="'+i+'">'+str+'</div>';
    }
    video_html += '</div>';
    ...
    display_element.querySelector('#jspsych-video-button-response-stimulus').onended = function(){
      ...
      if (trial.response_only_after_video){
        for(var i=0; i<btns.length; i++){
          btns[i].setAttribute('enabled', 'enabled');
        }
      }
    }
